...
| `zaproszenie.$token.tsx` | `/zaproszenie/:token` | loader, action, default | public | Konsumpcja zaproszenia: zakłada/odnawia konto podopiecznego, ustawia hasło, tworzy sesję, redirect na `/trener` lub `/podopieczny` (dalsze bramki — płatność, formularz startowy — przejmuje layout podopiecznego, patrz `podopieczny/_layout.tsx`). Gdy zaproszenie niosło `monthlyAmountGrosze` i Stripe jest skonfigurowany, best-effort zapisuje początkową kwotę subskrypcji (`setMonthlyAmount`). |
| `upload.wideo.tsx` | `/upload/wideo` | action | auth (podopieczny) | **Trasa zasobowa** (bez komponentu): JEDNO nagranie serii → `{ fileId, bytes, mimeType }`. Rozdziela wysyłkę pliku od zapisu sesji: szczyt pamięci JEDNEGO żądania to `MAX_VIDEO_UPLOAD_BYTES` i przestaje rosnąć z liczbą serii w planie (sufit per żądanie, NIE per proces — współbieżność nie jest ograniczona). Powtarza OBIE bramki z `podopieczny/_layout.tsx` (w tej samej kolejności: płatność przez `hasTraineeAppAccess` → 402, potem formularz startowy przez `hasPendingOnboarding` → 403), bo trasa leży poza tym layoutem — kto nie może zapisać treningu, nie może też wgrywać nagrań, które byłyby sierotami. `kind` jest stałą (`set_video`), `trainerId` wyłącznie z sesji. Rate limit per użytkownik (`RATE_LIMITS.upload`). Zwrócony `fileId` sam w sobie NIC nie uprawnia — właściciela weryfikuje dopiero zapis treningu. |
