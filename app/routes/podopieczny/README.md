...
|---|---|---|---|
| `_layout.tsx` | `/podopieczny` (layout) | loader, default | Wymaga roli podopiecznego; liczy logi/zdjęcia/sesje do nawigacji; badge „Konsultacje" = liczba terminów do potwierdzenia (`countPendingForTrainee`); badge „Płatności" = 1 gdy status `past_due`/`unpaid` lub `none` z ustawioną ceną. Topbar + sidenav + `<Outlet/>`. |
