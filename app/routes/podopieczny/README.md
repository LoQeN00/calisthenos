...
| `loguj.$sessionId.tsx` | `/podopieczny/loguj/:sessionId` | loader, action, ErrorBoundary, default | **Logowanie treningu**: per-seria reps/trudność/notatka + opcjonalne wideo (`multipart`); zapis przez `saveWorkoutLog`, wykrywanie nowych PR-ów. Kliencki twardy limit rozmiaru wideo (`maxUploadBytesFor("set_video")` z loadera) blokuje za duży plik przed wysłaniem. Wpisane serie są zapisywane jako szkic w `sessionStorage` (`lib/log-draft`) i przywracane po hydracji — chroni je, gdy wysyłka zerwie się na sieci. `ErrorBoundary` zamienia surowy błąd wysyłki na czytelny komunikat (powrót remontuje trasę i odtwarza szkic). Po udanym zapisie redirect niesie `?saved=<id sesji>`. |
