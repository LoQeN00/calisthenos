...
export const consultationCadence = pgEnum('consultation_cadence', ['weekly', 'biweekly', 'monthly']);
...
export const consultationActionItems = pgTable(
  ...
  (t) => ({
    uniq: uniqueIndex('consultation_action_items_consultation_ordinal_uniq').on(t.consultationId, t.ordinal),
  }),
);
...
export const skillVariations = pgTable(
  ...
  (t) => ({
    ...
    skillExerciseUniq: uniqueIndex('skill_variations_skill_exercise_uniq').on(t.skillId, t.exerciseId),
...
export const skillAdvancements = pgTable(
  ...
  (t) => ({
    traineeSkillIdx: index('skill_advancements_trainee_skill_idx').on(t.traineeId, t.skillId, t.advancedOn),
...
export const skillPrerequisites = pgTable(
  ...
  (t) => ({
    ...
    noSelfLoop: check('skill_prerequisites_no_self_loop', sql`${t.skillId} <> ${t.requiresSkillId}`),
  }),
);
