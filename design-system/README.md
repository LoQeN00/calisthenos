...
- **Warstwy jako płyty pełnej szerokości.** Każdy tier to well `--surface-2` z hairline'em, ciągnący się przez całą planszę. **Wcięcie pasów zostało wycofane 2026-07-25**: rzędy nie wykorzystywały zarezerwowanego miejsca, więc karty siedziały małe na pustych płytach. Masę niesie teraz sama warstwa, **rzymski numer** `I–IV` wykuty w jej rogu (`--ink` przy kryciu 0,055; na szczycie `--bg` przy 0,12) i atramentowa płyta `EKSPERT`.
- **Kierunek zależności jest na karcie, nie na krawędzi.** Karta, z której coś wyrasta, dostaje **stopkę** na górnej krawędzi; karta, która czegoś wymaga — **grot** na dolnej. Każda linia biegnie od stopki do grotu, więc dwa znaczniki opisują całą planszę i nie trzeba grotu na każdej krawędzi. Znaczniki są elementami DOM, nie `<marker>` w SVG — oś X planszy jest rozciągana i marker by się spłaszczył. Kolor: `--ok`, gdy droga jest już otwarta (prereq opanowany), inaczej `--line-2`.
- **Krawędzie są ortogonalne.** Łamana z zaokrąglonym zakrętem zamiast beziera: pion w wolnym korytarzu, przeskok w pasie tuż pod celem, wejście w spód karty. Pion nigdy nie przechodzi pod kartą, a dwa poziome odcinki w jednej przerwie nigdy się nie nakładają — to jest warunek czytelności, nie estetyka.
- **Linia szczytu.** Pozioma linia `--accent` na wysokości najwyższego tieru z co najmniej jedną opanowaną umiejętnością, z plakietką „TWÓJ SZCZYT · <TIER>". Jedyne miejsce, gdzie limonka wychodzi poza kartę — nadal znaczy postęp podopiecznego. Bez opanowanej umiejętności nie jest renderowana.
